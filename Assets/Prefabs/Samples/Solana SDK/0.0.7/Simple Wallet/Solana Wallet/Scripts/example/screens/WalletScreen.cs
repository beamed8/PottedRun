                ...
                foreach (var item in tokenAccounts)
                {
                    ...
                    if (_instantiatedTokens.All(t => t.TokenAccount.Account.Data.Parsed.Info.Mint != item.Account.Data.Parsed.Info.Mint))
                    {
                            ...
                            Web3.Instance.Wallet.ActiveRpcClient).AsUniTask().ContinueWith(nft =>
                        {
                            ...
                            // EDITED
                            // Debug.Log("nft metaplex data mint: " + nft.metaplexData.mint);
                            // if (HashlistController.instance.CompareHash(nft.metaplexData.mint))
                            ...
                            {
                                tkInstance.gameObject.SetActive(false);
                            }
                            // Debug.Log("nft metaplex data symbol: " + nft.metaplexData.data.symbol);
                            // Debug.Log("nft metaplex data name: " + nft.metaplexData.data.name);
                            // Debug.Log("tokeninstance tokenaccount publickey" + tkInstance.TokenAccount.PublicKey);
                            // EDITED
